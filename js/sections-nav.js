...
(function() {
    ...
    const CORRECT_PASSWORD_HASH = 'REMOVED'; 
    ...
    // Fits the device screen image correctly based on its aspect ratio:
    // - wider than the slot → contain (no side clipping)
    // - taller than the slot → cover + top (clips from bottom, not squished)
    function setupDeviceScreen(modal) {
        const screen = modal.querySelector('img.device-screen');
        if (!screen) return;
        const SLOT_RATIO = 600 / 375; // macbook screen slot
        const apply = () => {
            const imgRatio = screen.naturalWidth / screen.naturalHeight;
            if (imgRatio >= SLOT_RATIO) {
                screen.style.objectFit = 'contain';
                screen.style.objectPosition = '';
            } else {
                screen.style.objectFit = 'cover';
                screen.style.objectPosition = 'top';
            }
        };
        screen.complete ? apply() : screen.addEventListener('load', apply, { once: true });
    }
